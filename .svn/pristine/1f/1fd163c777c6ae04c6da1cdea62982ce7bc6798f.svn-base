    ...
    function TaskQueue(debounceWait) {
        ...
        function addTask(taskFunc) {
            return new Promise(function (resolve) {
                ...
                var runFunc = function () {
                    Promise.resolve(taskFunc()) // result might be async, coerce to promise
                        ...
                        .then(function () {
                            ...
                            // run the next taskmanage, if any
                            if (q.length) {
                                q[0]();
                            }
                        });
                ...
                // always put the taskmanage at the end of the queue, BEFORE running the taskmanage
                q.push(runFunc);

                // if it's the only taskmanage in the queue, run immediately
                if (q.length === 1) {
                    runFunc();
                }
            });
...
// TODO: use Emitter
    ...
    var HitDragListener = DragListener.extend({
        ...
        handleDragStart: function (ev) {
            ...
            // interface the initial hit the mouse is over
            ...
            var dragListener = new HitDragListener(this, {
                ...
                interactionEnd: function (ev, isCancelled) {
                    if (!isCancelled) {
                        ...
                        if (selectionSpan) {
                            // the selection will already have been rendered. just interface it
...
// We mixin DayTable, even though there is only a single row of days
    ...
    /* Methods for rendering SEGMENTS, pieces of content that live on the view
     ( this conf is no longer just for events )
     ----------------------------------------------------------------------------------------------------------------------*/
...
// TODO: document the structure and ordering of a FullCalendar locale conf
...
// Sets FullCalendar-specific translations. Will set the locales as the global default.
    FC.locale = function (localeCode, newFcOptions) {
        ...
        // always do this. newFcOptions can be undefined when initializing from i18n conf,
...
// configs, so make sure there are English fallbacks for these in the defaults conf.
